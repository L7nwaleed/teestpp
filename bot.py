...
api_id = os.environ.get("APP_ID", 15706456)
api_hash = os.environ.get("API_HASH", '2f7fa29bfdbdef20a8bf96522da9aa35')
token = os.environ.get("BOT_TOKEN" '5720532313:AAHWWtqafDhPHfClfKlQIzL3pIK3I0sm6D4')
dev = os.environ.get("DEV", 'lL7Nl')
...
@client.on(events.NewMessage(pattern="/eg", func=lambda x: x.is_group))
async def op(event):
  await event.reply("▾∮ عذرا البوت يعمل في الخاص فقط")
@client.on(events.NewMessage(pattern="/eg", func = lambda x: x.is_private))
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.respond("▾∮ استخدم /eg فقط")
